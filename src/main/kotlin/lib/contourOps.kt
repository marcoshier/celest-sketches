...
fun computeContours(img: ColorBuffer): List<ShapeContour> {
    ...
    var filtered = BinaryImageOps.erode8(binary, 0, null)
    filtered = BinaryImageOps.dilate8(filtered, 0, null)

    return BinaryImageOps.contour(filtered, ConnectRule.EIGHT, null).toShapeContours(external = false, internal = true, closed = true)
}
